	...
	rgbToHex(rgb) {
		if (rgb) {
			...
			if (rgbm && rgbm.length === 5) {
				return `#${`0${Math.round(parseFloat(rgbm[4], 10) * 255)
					.toString(16)
					.toUpperCase()}`.slice(-2)}${`0${parseInt(rgbm[1], 10).toString(16).toUpperCase()}`.slice(
					-2
				)}${`0${parseInt(rgbm[2], 10).toString(16).toUpperCase()}`.slice(-2)}${`0${parseInt(rgbm[3], 10)
					.toString(16)
					.toUpperCase()}`.slice(-2)}`;
			}
