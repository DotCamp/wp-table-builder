<template>
	<window-header-button :title="message" :style="{ cursor: count === 0 ? 'default' : 'pointer' }">
		<div class="wptb-table-data-associated-tables">
			<span class="wptb-table-data-associated-tables-inner-wrapper" @click.prevent="showTableListing"
				><span class="wptb-table-data-associated-tables-number" :data-plural="count > 0">{{ count }}</span>
				<span class="dashicons dashicons-editor-table wptb-table-data-header-icon"></span
			></span>
			<usage-list-modal
				:title="translationM('tables')"
				:visibility="listVisibility"
				@close="listVisibility = false"
			>
...
export default {
	components: { WindowHeaderButton, TableCard, UsageListModal },
	props: {
		associatedTables: {
			type: Array,
			default: () => [],
