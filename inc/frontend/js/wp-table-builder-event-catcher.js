...
(function assignToGlobal(global, factory) {
	...
	function WptbEventCatcher(mainTableQuery, cloneTableQuery) {
		...
		function isPartOfTable(identifierType, identifierClass, el) {
			...
			if (elementType === 'HTML' || !elementType) {
				return false;
			}
		...
		function EventCatcherSingle(cloneParentQuery) {
			 ...
			 * @param {any} options options for addEventListener
			 */
			this.addToCaptured = (key, eventName, eventCallback, options) => {
				...
				capturedEvents[key][eventName].push(currentEventObject);
			};
			...
			HTMLElement.prototype.addEventListener = function eventListener(eventName, callback, options = false) {
				...
				if (isWptbRelated) {
					...
					if (elementClone) {
						...
						vm.addToCaptured(captureId, eventName, callback, options);
					}
				}
					...
					if (Object.prototype.hasOwnProperty.call(allEvents, k)) {
						...
						Object.keys(allEvents[k]).map((e) => {
							if (Object.prototype.hasOwnProperty.call(allEvents[k], e)) {
								const callBackObjects = allEvents[k][e];

								// iterate callbacks of event name of registered element
								callBackObjects.map((c) => {
									registeredCloneElement._addEventListener(e, c.callback, c.options);
								});
							}
						});
					}
...
console.log('event catcher loaded');
